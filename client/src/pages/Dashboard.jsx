        ...
        <div className="min-h-screen bg-gradient-to-br from-amber-900 via-amber-800 to-blue-900">
            ...
            <div className="max-w-6xl mx-auto p-4 md:p-6">
                ...
                <div className="bg-white/95 rounded-2xl p-6 shadow-2xl mb-6">
                    <h1 className="text-2xl md:text-3xl font-bold text-amber-900">Welcome, {user.name}! 👋</h1>
                    <p className="text-gray-600 mt-1">@{user.username} • {user.role} • 💎 {user.connects} connects</p>
                    ...
                    <div className="mt-4 relative">
                        <input
                            ...
                            className="w-full p-4 border-2 border-amber-200 rounded-xl focus:border-blue-600 outline-none text-lg"
                        />
                        {searchTerm && (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                    <div className="bg-white/95 rounded-xl p-4 text-center shadow">
                        <div className="text-2xl font-bold text-amber-900">{jobs.length}</div>
                    ...
                    <div className="bg-white/95 rounded-xl p-4 text-center shadow">
                        <div className="text-2xl font-bold text-green-900">{user.connects || 0}</div>
                        <div className="text-xs text-gray-600">Connects</div>
                    </div>
                    <div className="bg-white/95 rounded-xl p-4 text-center shadow">
                        <div className="text-2xl font-bold text-purple-900">{myApps.length}</div>
                ...
                <div className="flex gap-2 mb-4 flex-wrap">
                    <button onClick={() => setActiveTab('all')} className={`px-4 py-2 rounded-xl text-sm font-semibold ${activeTab === 'all' ? 'bg-white text-amber-900' : 'bg-white/50 text-white'}`}>All</button>
                    <button onClick={() => setActiveTab('jobs')} className={`px-4 py-2 rounded-xl text-sm font-semibold ${activeTab === 'jobs' ? 'bg-white text-amber-900' : 'bg-white/50 text-white'}`}>📋 Jobs</button>
                    <button onClick={() => setActiveTab('posts')} className={`px-4 py-2 rounded-xl text-sm font-semibold ${activeTab === 'posts' ? 'bg-white text-amber-900' : 'bg-white/50 text-white'}`}>📝 Posts</button>
                    {user?.role === 'artist' && (
                        <button onClick={() => setActiveTab('myapps')} className={`px-4 py-2 rounded-xl text-sm font-semibold ${activeTab === 'myapps' ? 'bg-white text-amber-900' : 'bg-white/50 text-white'}`}>📋 My Applications</button>
                    )}
                </div>
                    ...
                    <div className="mb-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {displayedJobs.map(job => (
                                <div key={job._id} className="bg-white/95 rounded-2xl p-5 shadow-xl hover:shadow-2xl transition">
                                    <h3 className="font-bold text-amber-900 text-lg">{job.title}</h3>
                                    <p className="text-gray-600 text-sm mt-1 line-clamp-2">{job.description}</p>
                                    <div className="flex flex-wrap gap-2 mt-3 text-xs">
                                        <span className="bg-amber-100 text-amber-800 px-2 py-1 rounded-full">💰 ${job.budget}</span>
                                        <span className="bg-blue-100 text-blue-800 px-2 py-1 rounded-full">⏰ {job.deadline}</span>
                                        <span className="bg-purple-100 text-purple-800 px-2 py-1 rounded-full">⚡ {job.urgency}</span>
                                    </div>
                                    <p className="text-xs text-gray-500 mt-2">👔 {job.clientName}</p>
                                    ...
                                    <div className="flex gap-2 mt-4">
                                        {user?.role === 'artist' && job.status === 'open' && (
                                            <button
                                                ...
                                                className="bg-gradient-to-r from-amber-700 to-blue-700 text-white px-4 py-2 rounded-xl text-sm font-semibold w-full"
                                            >
                                        ...
                                        {user?.role === 'client' && job.client?._id === user._id && (
                                            <button
                                                ...
                                                className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-semibold w-full"
                                            >
                                                📋 View Applications
                                            </button>
                                        )}
                        ...
                        {displayedJobs.length === 0 && (
                            <div className="text-center py-8 bg-white/50 rounded-2xl">
                                <p className="text-white text-lg">No jobs found{searchTerm ? ` matching "${searchTerm}"` : ''}</p>
                            </div>
                        )}
                    </div>
                )}
                ...
                {(activeTab === 'all' || activeTab === 'posts') && (
                    <div className="mb-6">
                        <h2 className="text-xl font-bold text-white mb-4">📝 Portfolio Posts</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {posts.map(post => (
                                <div key={post._id} className="bg-white/95 rounded-2xl p-5 shadow-xl">
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500">@{post.user?.username} • {new Date(post.createdAt).toLocaleDateString()}</p>
                                        </div>
                                    </div>
                ...
                {(activeTab === 'all' || activeTab === 'myapps') && user?.role === 'artist' && (
                    <div className="mb-6">
                        <h2 className="text-xl font-bold text-white mb-4">📋 My Applications ({myApps.length})</h2>
                        {myApps.length === 0 ? (
                            <div className="bg-white/50 rounded-2xl p-8 text-center">
                                <p className="text-white">No applications yet. Browse jobs above!</p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                {myApps.map(app => {
                                    const job = app.job || {};
                                    return (
                                        <div key={app._id} className="bg-white/95 rounded-2xl p-5 shadow-xl">
                                            <div className="flex justify-between items-start mb-3">
                                                <div>
                                                    <h3 className="font-bold text-amber-900 text-lg">{job.title || 'Unknown Job'}</h3>
                                            ...
                                            <div className="grid grid-cols-3 gap-2 text-xs mb-3">
                                                <span className="bg-amber-50 px-2 py-1 rounded">💰 ${job.budget}</span>
                                                <span className="bg-blue-50 px-2 py-1 rounded">👔 {job.clientName}</span>
                                                <span className="bg-purple-50 px-2 py-1 rounded">💎 10 connects</span>
                                            </div>
                                                ...
                                                <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-3">
                                                    ...
                                                    <div className="flex gap-2 mt-3">
                                                        <button onClick={() => navigate('/chat')}
                                                            className="bg-green-500 text-white px-4 py-2 rounded-xl text-sm font-semibold">💬 Chat</button>
                                                        <button onClick={() => { setVideoCallUser(job.clientName || 'Client'); setShowVideoModal(true); }}
                                                            className="bg-blue-500 text-white px-4 py-2 rounded-xl text-sm font-semibold">📹 Video Call</button>
                                                    </div>
                                                </div>
                                            )}
                ...
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setShowApplyModal(false)}>
                    <div className="bg-white rounded-2xl p-6 max-w-lg w-full mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-bold text-amber-900">Apply for: {applyJobTitle}</h3>
                            <button onClick={() => setShowApplyModal(false)} className="text-gray-500 hover:text-gray-700 text-2xl">✕</button>
                        </div>

                        <p className="text-sm text-amber-700 mb-4">⚠️ This costs <strong>10 connects ($0.10)</strong></p>

                        <div className="mb-4">
                            <label className="block font-semibold text-amber-900 mb-2">Cover Letter *</label>
                            <textarea
                                value={coverLetter}
                        ...
                        <div className="mb-4">
                            <label className="block font-semibold text-amber-900 mb-2">Portfolio URL (optional)</label>
                            <input
                                type="url"
                                value={portfolioUrl}
                        ...
                        <div className="mb-4">
                            <label className="block font-semibold text-amber-900 mb-2">Upload Portfolio (audio/video)</label>
                            <input
                                type="file"
                                accept="audio/*,video/*"
                                onChange={(e) => setPortfolioFile(e.target.files[0])}
                                className="w-full p-3 border-2 border-amber-200 rounded-xl bg-white"
                            />
                            {portfolioFile && (
                        ...
                        <button
                            ...
                            disabled={applying}
                            className="w-full bg-gradient-to-r from-amber-700 to-blue-700 text-white p-3 rounded-xl font-semibold disabled:opacity-50"
                        >
                ...
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50" onClick={() => setShowVideoModal(false)}>
                    <div className="bg-gray-900 rounded-2xl p-6 max-w-2xl w-full mx-4" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex justify-center gap-4">
                            <button className="bg-gray-700 text-white px-4 py-2 rounded-xl text-sm">🎤 Mute</button>
                            <button className="bg-gray-700 text-white px-4 py-2 rounded-xl text-sm">📷 Stop Video</button>
                            <button onClick={() => setShowVideoModal(false)} className="bg-red-600 text-white px-4 py-2 rounded-xl text-sm">🔴 End Call</button>
                        </div>
                    </div>
                </div>
            )}
